...
void HelloWorld::addPlayers()
{
	...
	//设置第一个精灵的动画
	//创建一张贴图
	auto texture = Director::getInstance()->getTextureCache()->addImage("$shana_walk.png");
	...
	//攻击动画
	//$kanaIriya_beat
	auto texture0_1 = Director::getInstance()->getTextureCache()->addImage("$shana_beat.png");
	attack.reserve(11);
	for (int i = 0; i < 11; i++) {
		auto frame = SpriteFrame::createWithTexture(texture0_1, CC_RECT_PIXELS_TO_POINTS(Rect(90 * i, 0, 90, 68)));
		attack.pushBack(frame);
	}
	attack.pushBack(frame0);

	//死亡动画
	auto texture0_2 = Director::getInstance()->getTextureCache()->addImage("$shana_dead.png");
	dead.reserve(3);
	for (int i = 0; i < 3; i++) {
		auto frame = SpriteFrame::createWithTexture(texture0_2, CC_RECT_PIXELS_TO_POINTS(Rect(79 * i, 0, 79, 68)));
		dead.pushBack(frame);
	}

	//运动动画
	//$kanaIriya_forward
	auto texture0_3 = Director::getInstance()->getTextureCache()->addImage("$shana_forward.png");
	run.reserve(8);
	for (int i = 0; i < 8; i++) {
		auto frame = SpriteFrame::createWithTexture(texture0_3, CC_RECT_PIXELS_TO_POINTS(Rect(74 * i, 0, 74, 63)));
		run.pushBack(frame);
	}
	run.pushBack(frame0);
	/////////////////////////////////////
...
void HelloWorld::continueKeyUpdate(float f) {
	...
	case 0:
		if (player2->getPosition().x - 25 > origin.x) {
			player2->setPosition(player2->getPositionX() - 8, player2->getPositionY());
	...
	case 1:
		if (player2->getPosition().y + 25 < origin.y + visibleSize.height) {
			player2->setPosition(player2->getPositionX(), player2->getPositionY() + 8);
			player2->runAction(Sequence::create(moveAnimate2, NULL));
		}
		break;
		//右
